...
static esp_timer_handle_t auto_close_timer = NULL;
#define AUTO_CLOSE_DELAY_MS 3000  // 3 seconds

/* This is the button that is used for toggling the power */
#define BUTTON_GPIO          CONFIG_EXAMPLE_BOARD_BUTTON_GPIO
#define BUTTON_ACTIVE_LEVEL  0

/* This is the exit button for people inside to exit */
#define EXIT_BUTTON_GPIO     CONFIG_EXAMPLE_EXIT_BUTTON_GPIO
#define EXIT_BUTTON_ACTIVE_LEVEL  0
...
static void auto_close_timer_cb(void *arg)
{
    ESP_LOGI("app_driver", "Auto-closing door after 3 seconds");
    app_driver_set_state(false);
    esp_rmaker_param_update_and_report(
            esp_rmaker_device_get_param_by_name(switch_device, ESP_RMAKER_DEF_POWER_NAME),
            esp_rmaker_bool(false));
    
    /* Reset RC522 antenna để có thể quét thẻ lại */
    need_rc522_reset = true;
}

static void exit_btn_cb(void *arg, void *data)
{
    ESP_LOGI("app_driver", "Exit button pressed - opening door");
    
    /* Stop any existing timer */
    if (auto_close_timer) {
        esp_timer_stop(auto_close_timer);
    }
    
    /* Set power to ON to open the door */
    ...
    if (auto_close_timer) {
        esp_timer_start_once(auto_close_timer, AUTO_CLOSE_DELAY_MS * 1000);
    }
}
...
static void set_power_state(bool target)
{
    ...
    // Tắt LED để không xung đột với relay trên cùng GPIO
...
void app_driver_init()
{
    ...
    esp_timer_create_args_t timer_args = {
        .callback = auto_close_timer_cb,
        .arg = NULL,
        .name = "auto_close",
        .dispatch_method = ESP_TIMER_TASK,
    };
    esp_timer_create(&timer_args, &auto_close_timer);
    ...
    button_gpio_config_t exit_gpio_cfg = {
        .gpio_num = EXIT_BUTTON_GPIO,
        .active_level = EXIT_BUTTON_ACTIVE_LEVEL,
        .enable_power_save = false,
    };
    button_handle_t exit_btn_handle = NULL;
    if (iot_button_new_gpio_device(&btn_cfg, &exit_gpio_cfg, &exit_btn_handle) == ESP_OK && exit_btn_handle) {
        /* Register callback for exit button single click event */
        iot_button_register_cb(exit_btn_handle, BUTTON_SINGLE_CLICK, NULL, exit_btn_cb, NULL);
        ESP_LOGI("app_driver", "Exit button initialized on GPIO %d", EXIT_BUTTON_GPIO);
    }
