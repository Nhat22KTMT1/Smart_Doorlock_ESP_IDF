...
void PCD_Init(spi_device_handle_t spi){
    ...
    ESP_LOGI("RC522", "Starting PCD_Init...");
    
    // Hard Reset - TIMING GIỐNG ARDUINO
    gpio_set_level(PIN_NUM_RST, 0);
    vTaskDelay(10 / portTICK_PERIOD_MS);
    
    gpio_set_level(PIN_NUM_RST, 1);
    vTaskDelay(50 / portTICK_PERIOD_MS);  // Arduino dùng 50ms
    
    // Thử soft reset trước
    PCD_WriteRegister(spi, CommandReg, PCD_SoftReset);
    
    // Đợi soft reset hoàn tất (giống Arduino)
    uint8_t timeout = 0;
    do {
        vTaskDelay(10 / portTICK_PERIOD_MS);
        timeout++;
        if (timeout > 50) {  // Timeout 500ms
            ESP_LOGE("RC522", "Soft reset timeout!");
            break;
        }
    } while (PCD_ReadRegister(spi, CommandReg) & (1 << 4));  // PowerDown bit
    ...
    if (ver == 0x92) {
        ...
        printf("ERROR: No communication with MFRC522!\n");
        ...
        ver = PCD_ReadRegister(spi, VersionReg);
        printf("Retry - Version: 0x%02X\r\n", ver);
        ...
        printf("WARNING: Unknown version 0x%02X\n", ver);
    }
